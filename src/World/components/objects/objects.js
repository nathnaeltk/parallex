...
async function loadObjects() {
  ...
  if (objectID == 0) {
    const data = await loader.loadAsync('https://nathnaeltk.github.io/parallex/models/tie.glb');
    object = setupModel(data);
    object.scale.set(0.00002, 0.00002, 0.00002);
    object.position.set(0, 0, -0.05);
  } else if (objectID == 1) {
    const data = await loader.loadAsync('https://nathnaeltk.github.io/parallex/models/parrot.glb');
    object = setupModel(data);
    object.scale.set(0.15, 0.15, 0.15);
    object.position.set(0, 0, -0.02);
  } else {
    const data = await loader.loadAsync('https://nathnaeltk.github.io/parallex/models/vangogh.glb');
    object = setupModel(data);
